const songs = [
  {
    title: "Aku kamu yang lain",
    artist: "Raissa Anggiani",
    file: "music/Aku Kamu Yang Lain.mp3",

    cover: "photo/aku kamu yang lain.jpg",

lyrics: `
Tentang perasaan yang memudar
Seiring berjalannya waktu di dalam hidupku dan kamu

Tentang kisah kasih
Yang belum sempat terjaga
Cerobohnya kita pada masa itu
Ternyata tak sejalan
Walau sudah usahakan

Mungkin tak apa tuk berpisah
Selagi kamu baik-baik saja
Dan mungkin kau dipertemukan
Dengan aku aku yang lain

Pagi tadi kusapa mentari
Katanya salam rindu padamu yang terdalam
Ku jelaskan padanya akan kita

Ia pun bersedih akupun juga
Ternyata tak sejalan
Walau sudah usahakan
Mungkin tak apa tuk berpisah
Selagi kamu baik baik saja
Dan mungkin kau dipertemukan
Dengan aku aku yang lain
Aku bisa saja kembali padamu

Tapi jalan sudah lain
Bukan begitu ceritanya
Dan mungkin
Ku dipertemukan dengan
Kamu kamu yang lain
Dudu    `
  },

  {
    title: "& if we don't belong together",
    artist: "Raissa Anggiani",
    file: "music/& if we don't belong together.mp3",

    cover: "photo/& if we don't belong together.png",

lyrics: `
& if we don't belong together
i want you to know that i love you
with all the complexities that you have

let me tell you whats the worst part of it all
the rainbow starts to fade away
and that is how the rain comes from the tears

& i hope you never think of me
so i don't have to worry

— cause its hard to find somebody else
and do it all over again
all over again

& if we don't belong together
just promise me u'll be okay
then i'll be gone in peace forever more

& i hope you never think of me
so i don't have to worry

— cause its hard to find somebody else
and do it all over again
all over again

& i hope our memories starts to fade
and i can finally sleep

though ur still my favorite place on earth
but i am no good for you
i'm no good for you

although it hurts for me
cause i promise myself
to take care of you
and i failed
i failed

but if we do belong together
i promise i'll take care of us   `
  },

  {
    title: "Biru",
    artist: "Gabriella Fernaldi",
    file: "music/BIRU.mp3",

    cover: "photo/biru.jpg",

lyrics: `
Kuharap suatu saat nanti
Kita bisa saling menerima
Kurang dan lebihnya diri
Baik dan buruknya hati ini

Semoga bahagia selalu hidupmu
Doaku untukmu, bentuk cinta yang tulus
Semoga indah cerah selalu jalanmu
Laguku untukmu, bentuk cinta yang biru

Da, da-da-da
Da-da-da-da-da-da, da-da
Da-da-da

Kuharap suatu saat nanti
Mungkin di kehidupan yang lain
Kita terlahir sebagai sahabat saja
Hubungan ini terlalu menyakitkan

Semoga bahagia selalu hidupmu
Doaku untukmu, bentuk cinta yang tulus
Semoga indah cerah selalu jalanmu
Laguku untukmu, bentuk cinta yang biru

Ha-ah-ah
Ah-ah-ah-ah-ah
Ha-ah-ah-ah

Semoga bahagia selalu hidupmu
Doaku untukmu, bentuk cinta yang tulus
Semoga indah cerah selalu jalanmu
Laguku untukmu, bentuk cinta yang biru

Hm-uh-uh    `
  },

  {
    title: "Pamit",
    artist: "Tulus",
    file: "music/TULUS - Pamit (Official Lyric Video).mp3",

    cover: "photo/pamit.jpg",

lyrics: `
Tubuh saling bersandar
Ke arah mata angin berbeda
Kau menunggu datangnya malam
Saat ku menanti fajar

Sudah coba berbagai cara
Agar kita tetap bersama
Yang tersisa dari kisah ini hanya kau takut kuhilang
Perdebatan apapun menuju kata pisah
Jangan paksakan genggamanmu

Izinkan aku pergi dulu
Yang berubah hanya tak lagi kumilikmu
Kau masih bisa melihatku
Kau harus percaya kutetap teman baikmu

Sudah coba berbagai cara
Agar kita tetap bersama
Yang tersisa dari kisah ini hanya kau takut kuhilang
Perdebatan apapun menuju kata pisah
Jangan paksakan genggamanmu

Yang berubah hanya tak lagi kumilikmu
Kau harus percaya kutetap teman baikmu

Izinkan aku pergi dulu
Yang berubah hanya
Tak lagi kumilikmu
Kau masih bisa melihatku
Kau harus percaya
Kutetap teman baikmu
Oh    `
  },

{ 
    title: "Hati-hati di Jalan",
    artist: "Tulus",
    file: "music/Hati-Hati di Jalan.mp3",

    cover: "photo/hati hati di jalan.png",

lyrics: `
Perjalanan membawamu
Bertemu denganku
Ku bertemu kamu
Sepertimu yang kucari
Konon aku juga seperti yang kau cari

Kukira kita asam dan garam
Dan kita bertemu di belanga
Kisah yang ternyata tak seindah itu

Kukira kita akan bersama
Begitu banyak yang sama
Latarmu dan latarku

Kukira takkan ada kendala
Kukira inikan mudah
Kau aku jadi kita

Kasih sayangmu membekas
Redam kini sudah pijar istimewa
Entah apa maksud dunia
Tentang ujung cerita
Kita tak bersama

Semoga rindu ini menghilang
Konon katanya waktu sembuhkan
Akan adakah lagi yang sepertimu

Kukira kita akan bersama
Begitu banyak yang sama
Latarmu dan latarku
Kukira takkan ada kendala
Kukira inikan mudah
Kau aku jadi kita

Kau melanjutkan perjalananmu
Ku melanjutkan perjalananku
Uh uh uh

Kukira kita akan bersama
Begitu banyak yang sama
Latarmu dan latarku
Kukira takkan ada kendala
Kukira inikan mudah
Kau aku jadi kita
Kukira kita akan bersama

Hati-hati di jalan   `
  },

{
    title: "terima kasih",
    artist: "HAL",
    file: "music/terima kasih.mp3",

    cover: "photo/terima kasih.jpg",

lyrics: `
terkadang sendu tak selalu merakit luka
ia juga mengisyaratkan bahagia
seperti saat itu kala ku lihat dia
menggandeng tanganmu dengan penuh mesra

kepadamu dulu aku jatuh cinta
menanam asa bisa bersama sepanjang usia
saat itu engkau di tepian kota
aku masih sendiri kau sudah jadi miliknya

terima kasih atas segala rasa
pada hari itu pun aku turut bahagia
karena aku selalu tahu
menyukaimu bukan berarti selalu memilikimu

terima kasih atas segala rasa
pada hari itu pun aku turut bahagia
karena aku selalu tahu
menyukaimu bukan berarti selalu

terima kasih atas segala rasa
pada hari itu pun aku turut bahagia
karena aku selalu tahu
menyukaimu bukan berarti selalu memilikimu

yang terhebat dari waktu adalah karena
ia terus berputar secara sederhana
kita kembali bertemu di tepian kota
engkau sendiri lagi dan aku sudah berdua    `
  },

{ 
    title: "Mesra-mesraannya kecil-kecilan dulu",
    artist: "pal sriadi",
    file: "music/Mesra-mesraannya kecil-kecilan dulu.mp3",

    cover: "photo/mesra mesraan.jpg",

lyrics: `
Ba, sementara
Kita mesra-mesraannya
Kecil-kecilan dulu, ya
Tunggu sampai semua mereda

'Kan kukenalkan
Penampilan hujan di tempat lain
Pemandangan bagus di tempat yang jauh
Bukan yang di dekat rumah saja

Kita 'kan tangkap
Banyak kejadian yang menarik
Koleksi suasana asyik
Perasaan-p'rasaan yang baik
Cintanya besar-besaran
Meski mesranya kecil-kecilan

Ba, sementara
Kita mesra-mesraannya
Kecil-kecilan dulu, ya
Tunggu sampai semua mereda

Baju pergimu
Jangan kekecilan dulu
Kalau iya, nanti beli baru
Kar'na engkau tiba-tiba besar

Kita 'kan tangkap
Kejadian yang menarik
Koleksi suasana asyik
Perasaan-p'rasaan yang baik
Cintanya besar-besaran
Meski mesranya kecil-kecilan   `
  },

{ 
    title: "I Love u so",
    artist: "The Walters",
    file: "music/I Love You So.mp3",

    cover: "photo/i love you so.jpg",

lyrics: `
I just need someone in my life to give it structure
To handle all the selfish ways I'd spend my time without her
You're everything I want, but I can't deal with all your lovers
You're saying I'm the one, but it's your actions that speak louder
Giving me love when you are down and need another
I've got to get away and let you go, I've got to get over

But I love you so (ooh-ooh-ooh)
I love you so (ooh-ooh-ooh)
I love you so (ooh-ooh-ooh)
I love you so

I'm gonna pack my things and leave you behind
This feeling's old, and I know that I've made up my mind
I hope you feel what I felt when you shattered my soul
'Cause you were cruel, and I'm a fool, so please, let me go

But I love you so (please, let me go)
I love you so (please, let me go)
I love you so (please, let me go)
I love you so

Ooh-ooh-ooh
Ooh-ooh-ooh-ooh
Ooh-ooh-ooh
Ooh-ooh-ooh-ooh   `
  },

{ 
    title: "Sorai",
    artist: "Nadin Amizah",
    file: "music/Sorai.mp3",

    cover: "photo/sorai.png",

lyrics: `
Langit dan laut saling membantu
Mencipta awan hujan pun turun
Ketika dunia saling membantu
Lihat cinta mana yang tak jadi satu

Kau memang manusia sedikit kata
Bolehkah aku yang berbicara
Kau memang manusia tak kasat rasa
Biar aku yang mengemban cinta

Awan dan alam saling bersentuh (bersentuh)
Mencipta hangat kau pun tersenyum
Ketika itu kulihat syahdu
Lihat hati mana yang tak akan jatuh

Kau memang manusia sedikit kata
Bolehkah aku yang berbicara
Kau memang manusia tak kasat rasa
Biar aku yang mengemban cinta

Kau dan aku saling membantu
Membasuh hati yang pernah pilu
Mungkin akhirnya tak jadi satu
Namun bersorai pernah bertemu   `
  },

{ 
    title: "Semua Aku Dirayakan",
    artist: "Nadin Amizah",
    file: "music/Semua Aku Dirayakan.mp3",

    cover: "photo/semua aku dirayakan.png",

lyrics: `
Terima kasih katanya
Semua aku dirayakan
Jangan menangis ku dibuai
Sampai tenang

Diciumnya api marahku
Ternyata kacau bisa luluh
Jangan menangis biar tenang
Ku dibuai

Jika malam datang
Dan takut menyerang
Kau genggam apa yang kuragukan

Tiada yang bilang
Badainya kan reda
Berhadapan dengan cahaya yang kerap membutakan
Tiada yang bilang
Jawaban kan datang
Jauh dari seram yang selama ini telah kubayangkan
Semua aku dirayakan

Hatiku seberat dunia
Semua bentuknya kau rayakan
Menangis pun kau penuh tenang
Ku dibuai

Jika malam datang
Dan takut menyerang
Kau genggam apa yang kuragukan uh oh hu

Tiada yang bilang
Badainya kan reda
Berhadapan dengan cahaya yang kerap membutakan hu
Tiada yang bilang
Jawaban kan datang
Jauh dari seram yang selama ini telah kubayangkan
Semua aku dirayakan hu oh   `
  },

{ 
    title: "Di Akhir Perang",
    artist: "Nadin Amizah",
    file: "music/Di Akhir Perang.mp3",

    cover: "photo/di akhir perang.png",

lyrics: `
Perlahan akan
Ku ajarkan cara
Menanam menuai
Baik buruk di dunia

Ku warnai tanganmu yang mati
Biar kau lihat dunia tak lagi menyakiti

Dan ku bisikkan asal kau tahu bagaimana
Rasanya bahagia sepenuhnya sampai
Ku merasa lega kau merasa lega
Ku sampai di sana

Perlahan telah
Kau ajarkan cara
Menerima rasa
Baik buruk yang kupunya

Kau panggil jahat yang menyelimuti
Sampai kutahu dunia tak lagi menyakiti
Dan kubisikkan asal kau tahu bagaimana
Rasanya bahagia sepenuhnya sampai
Ku merasa lega kau merasa lega
Ku sampai di sana

Perang telah usai
Perang telah usai
Aku bisa pulang
Ku baringkan panah dan berteriak
Menang

Dan ku bisikkan asal kau tahu bagaimana
Rasanya bahagia sepenuhnya sampai
Ku merasa lega kau merasa lega
Ku sampai di sana kau sampai di sana
Ku sampai di sana kau sampai di sana   `
  },

{ 
    title: "Walking in the Wind",
    artist: "one direction",
    file: "music/Walking in the Wind.mp3",

    cover: "photo/walking in the wind.png",

lyrics: `
A week ago you said to me
"Do you believe I'll never be too far?"
If you're lost, just look for me
You'll find me in the region of the summer stars

The fact that we can sit right here and say goodbye
Means we've already won
A necessity for apologies between you and me
Baby, there is none

We had some good times, didn't we?
We had some good tricks up our sleeve
Goodbyes are bittersweet
But it's not the end
I'll see your face again

And you will find me
Yeah, you will find me
In places that we've never been
For reasons we don't understand
Walking in the wind
Walking in the wind

Yesterday I went out to celebrate the birthday of a friend
But as we raised our glasses up to make a toast
I realised you were missing

We had some good times, didn't we?
We wore our hearts out on our sleeve
Goodbyes are bittersweet
But it's not the end
I'll see your face again

And you will find me
Yeah, you will find me
In places that we've never been
For reasons we don't understand
Walking in the wind
Walking in the wind

And I know we'll be alright, child
Just close your eyes and see
And I'll be by your side
Any time you're needing me
Oh, yeah

And you will find me
Yeah, you will find me
In places that we've never been
For reasons we don't understand
Walking in the wind
Walking in the wind
Walking in the wind   `
  },

{ 
    title: "Take A Chance With Me",
    artist: "NIKI",
    file: "music/Take A Chance With Me.mp3",

    cover: "photo/take a chance with me.jpg",

lyrics: `
His laugh you'd die for, his laugh you'd die for
The kind that colors the sky
Heart intangible, slips away faster than
Dandelion fluff in the sunlight
And he's got swirls of passion in his eyes
Uncoverin' the dreams, he dreams at night
As much and hard as he tries to hide
I can see right through, see right through

His voice you'd melt for, he says my name like
I'd fade away somehow if he's too loud
What I would give for me to get my feet
Back on the ground, head off the clouds
I laugh at how we're polar opposites
I read him like a book, and he's a clueless little kid
Doesn't know that I'd stop time and space
Just to make him smile, make him smile

Oh, why can't we for once
Say what we want, say what we feel?
Oh, why can't you for once
Disregard the world, and run to what you know is real?
Take a chance with me, take a chance with me

Ooh-ooh, ooh-ooh
Ooh-ooh, ooh-ooh

His kiss you'd kill for, just one and you're done for
Electricity surgin' in the air
He drives me crazy, it's so beyond me
How he'd look me dead in the eye and stay unaware

That I'm hopelessly captivated
By a boy who thinks love's overrated
How did I get myself in this arrangement?
It baffles me, too, baffles me, too

Oh, why can't we for once
Say what we want, say what we feel?
Oh, why can't you for once
Disregard the world, and run to what you know is real?

In the end we only regret the chances we didn't take
I'll be your safety net, so why not raise the stakes?
And I can hear your heart from across the room
Pulsin' through my veins, I know you need this too
Lie to me all you please, I can see right through
See right through

Oh, why can't we for once
Say what we want, say what we feel?
Oh, why can't you for once
Disregard the world, and run to what you know is real?

Oh, why can't we for once
Say what we want, say what we feel?
Oh, why can't you for once
Disregard the world, and run to what you know is real?
Take a chance with me, take a chance with me
Ooh-ah, oh-oh, oh   `
  },

{ 
    title: "La La Lost You",
    artist: "NIKI",
    file: "music/La La Lost You.mp3",

    cover: "photo/la la lost you.jpg",

lyrics: `
While I'm on Sunset, are you on the subway?
While I drive, are you gettin' on the L-train?
I mean, Manhattan's nice, but so are Malibu nights
You would know if you stayed, you would know if you put up a fight
Your toes turn blue in winter, I'm gettin' red, rum
Does the trick for all of the things left unsaid, I'm
Missin' our drunken 2 A.M. strolls in K-Town
Now you're chasing fake highs in the Upper West Side

And fuckin' on Brooklyns in Brooklyn
Your Chelseas in Chelsea
Hope that eases the pain, so you remember to miss me
And you sold your car, now you walk for miles
Bet your feet feel numb
(Crosswalks in my mind are shaky, so please hold on tight)

All my demons run wild
All my demons have your smile
In the city of angels, in the city of angels
Hope New York holds you
Hope it holds you like I do
While my demons stay faithful
In the city of angels

Summer's endin' now and the nights are coolin' down
Remember last winter when we would drive around?
Silverlake, Hollywood, pretty little white lies got me good
Thought this was love, I was misunderstood, mmm

Feelin' low on the low, drivin' through NoHo
If I'm honest, I'd call, but I'm trying to let go
And I hope you're happy, livin' life in taxis
But you'll always have me, you'll always have me

All my demons run wild
All my demons have your smile
In the city of angels, in the city of angels
Hope New York holds you
Hope it holds you like I do
While my demons stay faithful
In the city of angels   `
  },

{ 
    title: "JALAN KELUAR",
    artist: "Nuca",
    file: "music/JALAN KELUAR.mp3",

    cover: "photo/jalan keluar.jpg",

lyrics: `
Sudah tak ada lagi kurasakan
Hangat yang dulu slalu kau berikan
Apakah kita berada di ujung jalan
Hanya menunggu hingga tak sejalan

Mungkin berpisah jalan keluarnya
Melepaskan erat genggaman kita
Tak ada lagi yang harus terluka
Kuingin kita sama sama bahagia

Mungkin inilah akhir ceritanya
Tak sama dengan yang di harapkan
Takdir memang sempat menyatukan
Tapi tangan kita yang memisahkan

Apa yang sebenarnya kau harapkan
Lelah dengan semua yang kau inginkan
Oh tak mungkin lagi cinta ini kan terulang
Kita menunggu hingga tak sejalan

Mungkin berpisah jalan keluarnya
Melepaskan erat genggaman kita
Tak ada lagi yang harus terluka
Kuingin kita sama sama bahagia

Mungkin inilah akhir ceritanya
Tak sama dengan yang di harapkan
Takdir memang sempat menyatukan
Tapi tangan kita yang memisahkan

Mungkin berpisah jalan keluarnya
Melepaskan erat genggaman kita
Tak ada lagi yang harus terluka
Kuingin kita sama sama bahagia

Mungkin inilah akhir ceritanya
Tak sama dengan yang di harapkan
Takdir memang sempat menyatukan
Tapi tangan kita yang memisahkan

Tak sama dengan yang di harapkan
Takdir memang sempat menyatukan
Tapi tangan kita yang memisahkan

Mungkin seharusnya tak bersama   `
  },

{ 
    title: "Abadi",
    artist: "Dendi Nata",
    file: "music/Abadi.mp3",

    cover: "photo/abadi.jpg",

lyrics: `
Aku terdiam berteman sepi
Bumi berputar, hanya sendiri
Lalu untuk apa lagi
Jalani ini?

Dia menghilang, memberikan sunyi
Nada yang terdengar tak akan berarti
Tapi wanginya selalu
Ada di sini

Bulan pun berganti
Harumnya telah pergi
Aku menanti
Aku tak mengerti

Biar pun kita hilang, tak lagi bersama
Tak lagi bercinta berdua
Masih saja kupandang wajahnya yang hilang
Tak akan berganti, dia abadi
Abadi untuk selamanya, selama-lamanya

Oh-oh, oh, oh-ho-oh
Uh-uh

Biar pun kita hilang, tak lagi bersama
Tak lagi bercinta berdua
Masih saja kupandang wajahnya yang hilang
Tak akan berganti, dia-

Dan biar pun kita hilang, tak lagi bersama
Tak lagi bercinta berdua
Masih saja kupandang wajahnya yang hilang
Tak akan berganti, dia abadi
Abadi untuk selamanya, abadi
Abadi untuk selamanya

Aku terdiam, berteman sepi   `
  },

  ];

let songIndex = 0;

const audio = document.getElementById("audio");
const title = document.getElementById("title");
const artist = document.getElementById("artist");
const lyrics = document.getElementById("lyrics");
const playBtn = document.getElementById("play");
const progress = document.getElementById("progress");
const currentTimeEl = document.getElementById("current-time");
const durationEl = document.getElementById("duration");
const playlist = document.getElementById("playlist");
const cover = document.getElementById("cover");

function loadSong(index){

  title.innerText = songs[index].title;
  artist.innerText = songs[index].artist;
  lyrics.innerText = songs[index].lyrics || "Lirik belum tersedia";

  cover.src = songs[index].cover;

  audio.src = songs[index].file;
}

loadSong(songIndex);
audio.play();
renderPlaylist();

function formatTime(time){

  if(isNaN(time)){
    return "0:00";
  }

  const minutes = Math.floor(time / 60);

  let seconds = Math.floor(time % 60);

  if(seconds < 10){
    seconds = "0" + seconds;
  }

  return `${minutes}:${seconds}`;
}
let isDragging = false;

progress.addEventListener("mousedown", () => {
  isDragging = true;
});

progress.addEventListener("mouseup", () => {

  const duration = audio.duration;

  audio.currentTime = (progress.value / 100) * duration;

  isDragging = false;
});

audio.addEventListener("timeupdate", () => {

  currentTimeEl.innerText =
    formatTime(audio.currentTime);

  durationEl.innerText =
    formatTime(audio.duration);

  if(!isDragging){

    const duration = audio.duration;
    const currentTime = audio.currentTime;

    const progressPercent =
      (currentTime / duration) * 100;

    progress.value = progressPercent;
    progress.style.background = `
linear-gradient(
  to right,
  #1db954 ${progress.value}%,
  #444 ${progress.value}%
)
`;
  }
});

function selectSong(index){

  songIndex = index;

  loadSong(songIndex);

  renderPlaylist();

  audio.play();

  playBtn.innerText = "⏸";
}

function playPause(){

  const vinyl = document.querySelector(".vinyl");

  if(audio.paused){

    audio.play();
    playBtn.innerText = "⏸";

    vinyl.style.animationPlayState = "running";

  } else {

    audio.pause();
    playBtn.innerText = "▶";

    vinyl.style.animationPlayState = "paused";
  }

}

function nextSong(){

  songIndex++;

  if(songIndex >= songs.length){
    songIndex = 0;
  }

  loadSong(songIndex);
  renderPlaylist();
  audio.play();

  const vinyl = document.querySelector(".vinyl");
  vinyl.style.animationPlayState = "running";

  playBtn.innerText = "⏸";
}

function prevSong(){

  songIndex--;

  if(songIndex < 0){
    songIndex = songs.length - 1;
  }

  loadSong(songIndex);
  audio.play();
  renderPlaylist();

  playBtn.innerText = "⏸";

}

  function renderPlaylist(){

  playlist.innerHTML = "";

  songs.forEach((song, index) => {

    playlist.innerHTML += `
      <div 
        class="song-item ${index === songIndex ? 'active-song' : ''}"
        onclick="selectSong(${index})">

        <img 
          src="${song.cover}" 
          class="playlist-cover">

        <div class="song-info">

          <div class="playlist-title">
            ${song.title}
          </div>

          <div class="playlist-artist">
            ${song.artist}
          </div>

        </div>

      </div>
    `;

  });

}